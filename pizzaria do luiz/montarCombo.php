...
<html>
    ...
    <body>
        ...
        <div class="pizzas">
            <form class="MontarC" method="post" action="cart.php?action=montarcombo">
                    ...
                    <select name="item_name" id="tamanhoPizza" onchange="showHideFlavors()">
                        ...
                        $sqlPizzas = "SELECT name, price FROM produtos WHERE id_categoria = 3 AND status = 1 ORDER BY name ASC";
                        $resultPizzas = $conn->query($sqlPizzas);

                        if ($resultPizzas->num_rows > 0) {
                            while ($rowPizza = $resultPizzas->fetch_assoc()) {
                                echo '<option value="' . $rowPizza['name'] . '">Pizza ' . $rowPizza['name'] . ' (com 1 refrigerante) R$' . $rowPizza['price'] . '</option>';
                            }
                        }
                    ...
                    <script>
                        function disableCheckboxes() {
                            ...
                            const refriCheckboxes = document.querySelectorAll('.refriCheckbox');
                            ...
                            refriCheckboxes.forEach(checkbox => {
                                checkbox.disabled = true;
                            });
                ...
                <div class="saborEborda"> 
                            ...
                            <label for="sabor">
                                ...
                                <div id="saboresContainer">
                                    ...
                                    // Sabores doces
                                    $sqlDoces = "SELECT * FROM montar WHERE id_variedades = 5";
                                    $resultDoces = $conn->query($sqlDoces);

                                    if ($resultDoces->num_rows > 0) {
                                        echo '<h4>Salgados:</h4>';
                                        while ($rowDoces = $resultDoces->fetch_assoc()) {
                                            echo '<input type="checkbox" class="saborCheckbox saborDoceCheckbox" value="' . $rowDoces["nome"] . '"> ' . $rowDoces["nome"] . '<br>';
                                        }
                                    }

                                    // Sabores salgados
                                    $sqlSalgados = "SELECT * FROM montar WHERE id_variedades = 6";
                        ...
                        <h3 style="color: black;">
                            <span style="color: red;">SELECIONE</span> a borda 🔻
                        </h3>
                            ...
                            <div id="bordasContainer">
                                <?
                                    $conn = new mysqli("robb0254.publiccloud.com.br", "calor_developer", "@Samsung2023", "calorysistemas_delivery");
                                //$conn = new mysqli("localhost", "root", "", "calorysistemas_delivery.");
                                if ($conn->connect_error) {
                                    die("Falha na conexão: " . $conn->connect_error);
                                }
                                //sabores doces
                                $sql = "SELECT * FROM montar WHERE id_variedades = 4";

                                $result = $conn->query($sql);

                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        echo '<input type="checkbox" class="bordasCheckbox" value="' . $row["nome"] . '"> ' . $row["nome"] . '<br>';
                                    }
                                }
                        ...
                        <label for="refri">
                            <span style="color: #800000;">
                                <h3 style="color: #800000;">REFRIGERANTE:</h3>
                            </span>
                        </label>
                        <h3 style="color: black;">
                            <span style="color: red;">SELECIONE</span> o seu refrigerante 🔻
                        </h3>
                        

                            <!-- Adicione um container para os checkboxes -->
                            <div id="refriContainer">
                                <?
                                    $conn = new mysqli("robb0254.publiccloud.com.br", "calor_developer", "@Samsung2023", "calorysistemas_delivery");
                                //$conn = new mysqli("localhost", "root", "", "calorysistemas_delivery.");
                                if ($conn->connect_error) {
                                ...
                                //sabores doces
                                $sql = "SELECT * FROM montar WHERE id_variedades = 7";

                                $result = $conn->query($sql);

                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        echo '<input type="checkbox" class="refriCheckbox" value="' . $row["nome"] . '"> ' . $row["nome"] . '<br>';
                                    }
                                }
                        ...
                        <input type="hidden" style="width: 30%; padding: 5px;" class="form-select" name="refri" id="refri">
                            <p id="refriSelecionados" style="color: black;"></p>
                        

                        <script>
                            const refriCheckboxes = document.querySelectorAll('.refriCheckbox');
                            const refriInput = document.getElementById('refri');
                            const refriSelecionados = document.getElementById('refriSelecionados');

                            refriCheckboxes.forEach(checkbox => {
                                checkbox.addEventListener('change', () => {
                                    const refriSelecionadosArray = Array.from(refriCheckboxes)
                                        .filter(checkbox => checkbox.checked)
                                        .map(checkbox => checkbox.value);


                                    refriInput.value = refriSelecionadosArray.join('* ');
                                });
                            });
                            ...
                            refriCheckboxes.forEach(checkbox => {
                                checkbox.addEventListener('change', () => {
                                    const checkedRefri = document.querySelectorAll('.refriCheckbox:checked');

                                    if (checkedRefri.length >= maxRefri) {
                                        refriCheckboxes.forEach(cb => {
                                            if (!cb.checked) {
                                                cb.disabled = true;
                                            }
                                        });
                    ...
                    <input class="my-btn" name="montarcombo" type="submit" value="Finalizar Combo">
                </div>
                </form>
        </div><br><br>
